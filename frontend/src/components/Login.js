...
const Login = ({ role }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const endpoint = role === 'student' ? '/api/auth/student-login' : '/api/auth/faculty-login';
      const data = role === 'student' ? { email, password, rollNumber } : { email, password, facultyId };
      const response = await axios.post(`${process.env.REACT_APP_API_URL}${endpoint}`, data);
      localStorage.setItem('token', response.data.token);
      toast.success(`Logged in as ${role}!`);
      navigate(role === 'student' ? '/dashboard' : '/faculty-dashboard');
    } catch (err) {
